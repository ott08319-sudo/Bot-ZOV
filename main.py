...
async def get_stats(user_id):
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        await db.execute("PRAGMA busy_timeout=5000;")
        async with db.execute("SELECT given, done FROM user_stats WHERE user_id=?", (user_id,)) as c:
            row = await c.fetchone()
            if row:
                return {"given": row[0], "done": row[1]}
            return {"given": 0, "done": 0}


async def save_sponsor(user_id, service, aid, link, reward, signature=None):
    """Сохраняет задание. Возвращает 1 если новое, 0 если уже было."""
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        await db.execute("PRAGMA busy_timeout=5000;")
        async with db.execute(
            "SELECT status FROM sponsor_tasks WHERE user_id=? AND service=? AND assignment_id=?",
            (user_id, service, aid)) as c:
            row = await c.fetchone()
        if row:
            # Уже есть запись — не показываем снова (даже если subscribed или unsubscribed)
            return 0
...
async def adm_stats(cq: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        ...
        async with db.execute("SELECT COALESCE(SUM(given),0), COALESCE(SUM(done),0) FROM user_stats") as c:
            g, d = await c.fetchone()
    await cq.answer()
    await safe_edit(cq,
        f"📊 <b>Статистика</b>\n\n"
        f"👥 Юзеров: <b>{total}</b>\n"
