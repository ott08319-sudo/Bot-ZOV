...
def flyer_client() -> Flyer | None:
    global _flyer
    if _flyer is None and FLYER_API_KEY:
        try:
            _flyer = Flyer(FLYER_API_KEY)
        except Exception as e:
            logging.error(f"Flyer init: {e}")
    return _flyer
...
# ========== API: FLYER (через библиотеку flyerapi) ==========
FLYER_INCOMPLETE = ("incomplete", "abort")

async def get_flyer(user_id, lang="ru"):
    """Возвращает список (signature, link) для Flyer sub-ключа."""
    f = flyer_client()
    if not f:
        return []
    try:
        tasks = await f.get_tasks(user_id=user_id, language_code=lang, limit=10)
    except FlyerAPIError as e:
        logging.warning(f"Flyer get_tasks API error: {e}")
        return []
    except Exception as e:
    ...
    for task in tasks:
        ...
        if status not in FLYER_INCOMPLETE:
            continue
        sig = task.get("signature")
        links = task.get("links") or []
        if sig and links:
            # берём первую ссылку из массива
            link = links[0]
            if link:
                result.append((sig, link))
    logging.info(f"Flyer tasks for {user_id}: {len(result)} incomplete")
    return result


async def check_flyer(signature):
    """Проверка статуса одного задания Flyer."""
    f = flyer_client()
    if not f or not signature:
        return False
    try:
        status = await f.check_task(signature=signature)
        logging.info(f"Flyer check_task {signature}: {status}")
        return status in ("complete", "waiting")
    except FlyerAPIError as e:
        logging.warning(f"Flyer check_task API error: {e}")
        return False
    except Exception as e:
...
async def collect_sponsors(user):
    ...
    # FLYER через библиотеку flyerapi
    for sig, link in await get_flyer(uid, lang):
        await save_sponsor(uid, "flyer", sig, link, reward, sig)
        links.append(link)
...
async def diagnose_flyer():
    """Диагностика Flyer через библиотеку."""
    f = flyer_client()
    if not f:
        return "❌ <b>Flyer</b> — ключ не задан"
    test_uid = ADMIN_ID if ADMIN_ID else 1
    t0 = time.time()
    try:
        tasks = await f.get_tasks(user_id=test_uid, language_code="ru", limit=5)
        dt = int((time.time() - t0) * 1000)
        if not tasks:
            return f"🟡 <b>Flyer</b> — ключ ок, 0 заданий ({dt}мс)"
        incomplete = [t for t in tasks if t.get("status") in FLYER_INCOMPLETE]
        return f"✅ <b>Flyer</b> — <b>{len(tasks)}</b> заданий ({len(incomplete)} невыполненных) ({dt}мс)"
    except FlyerAPIError as e:
        dt = int((time.time() - t0) * 1000)
        return f"⚠️ <b>Flyer</b> — API error: <code>{str(e)[:150]}</code> ({dt}мс)"
    except Exception as e:
        dt = int((time.time() - t0) * 1000)
        return f"❌ <b>Flyer</b> — {str(e)[:150]} ({dt}мс)"
...
async def main():
    await init_db()
    await start_web()
    flyer_client()  # инициализация Flyer
