...
def fmt_money(value: float) -> str:
    """Красивый формат: 0.004 / 0.01 / 0.5"""
...
async def check_flyer(signature):
    ...
    try:
        async with s.post(url, json=payload) as r:
            ...
            return status in ("completed", "subscribed", "done", True)
    except Exception as e:
...
async def collect_sponsors(user):
    ...
    for x in await get_flyer(uid, lang):
        ...
        sig = x.get("signature")
        if link:
            await save_sponsor(uid, "flyer", str(x.get("id")), link, reward, sig)
            links.append(link)
...
async def remind_lazy():
    """Напоминает юзерам с pending-заданиями."""
    while True:
