...
async def get_user(user_id):
    ...
    async with pool.acquire() as db:
        row = await db.fetchrow(
            "SELECT balance, referrer_id, referred_earned FROM users WHERE user_id=$1", user_id)
        return row
...
async def check_all(user_id):
    ...
    if await check_tgrass(user_id):
        ...
        async with pool.acquire() as db:
            row = await db.fetchrow(
                "SELECT COUNT(*) as cnt, COALESCE(SUM(reward),0) as sm FROM sponsor_tasks WHERE user_id=$1 AND service='tgrass' AND status!='subscribed'",
                user_id)
            await db.execute(
                "UPDATE sponsor_tasks SET status='subscribed' WHERE user_id=$1 AND service='tgrass'",
                user_id)
            done_count += row["cnt"]; done_sum += row["sm"]

    if await check_botohub(user_id):
        pool = await get_pool()
        async with pool.acquire() as db:
            row = await db.fetchrow(
                "SELECT COUNT(*) as cnt, COALESCE(SUM(reward),0) as sm FROM sponsor_tasks WHERE user_id=$1 AND service='botohub' AND status!='subscribed'",
                user_id)
            await db.execute(
