...
RENDER_URL = os.getenv("RENDER_URL", "")  # напр. https://bot-zov.onrender.com
...
async def safe_edit_markup(cq: types.CallbackQuery, markup, **kwargs):
    """edit_reply_markup без падения."""
    try:
        await cq.message.edit_reply_markup(reply_markup=markup, **kwargs)
    except TelegramBadRequest as e:
        if "message is not modified" in str(e):
            return
        logging.error(f"safe_edit_markup: {e}")
    except Exception as e:
        logging.error(f"safe_edit_markup: {e}")
...
async def get_flyer(user_id, lang="ru"):
    ...
    s = await http()
    try:
        async with s.post("https://api.flyerhubs.com/v1/tasks",
            json={"key": FLYER_API_KEY, "user_id": user_id, "language_code": lang,
                  "limit": await get_max_sponsors()}) as r:
            d = await r.json()
            if not d.get("error"): return d.get("result", [])
    except Exception as e:
...
async def check_flyer(signature):
    ...
    s = await http()
    try:
        async with s.post("https://api.flyerhubs.com/v1/check_task",
            json={"key": FLYER_API_KEY, "signature": signature}) as r:
            d = await r.json()
            if not d.get("error"): return d.get("result") in ("completed", "subscribed")
    except Exception as e:
...
async def collect_sponsors(user):
    ...
    for x in await get_flyer(uid, lang):
        ...
        if link:
            await save_sponsor(uid, "flyer", str(x.get("id")), link, reward, x.get("signature"))
            links.append(link)
...
async def run_diagnostics():
    ...
    results = await asyncio.gather(
        ...
        diagnose_service("Flyer", FLYER_API_KEY,
            ...
            {"key": FLYER_API_KEY, "user_id": test_uid, "language_code": "ru", "limit": max_sp}),
...
async def cb_earn(cq: types.CallbackQuery):
    ...
    logging.info(f"EARN pressed by {cq.from_user.id}")
    links = await collect_sponsors(cq.from_user)
    logging.info(f"Collected {len(links)} links for {cq.from_user.id}")

    if not links:
        await safe_edit(cq, "😕 Заданий нет, попробуй позже.", reply_markup=main_menu())
        return
    await safe_edit(cq,
        f"🎯 Подпишись на <b>{len(links)}</b> канал(ов).\n"
...
async def adm_reward(cq: types.CallbackQuery):
    ...
    await safe_edit(cq,
        ...
        f"Отправь новое значение (например: <code>0.01</code>)")
...
async def self_ping():
    """Стучимся сами в себя каждые 4 минуты, чтобы Render Free не усыпил."""
    if not RENDER_URL:
