...
# Дефолтные значения (меняются через админку, хранятся в БД)
...
async def register_user(user_id, referrer_id=None):
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        ...
        async with db.execute("SELECT user_id FROM users WHERE user_id=?", (user_id,)) as c:
            if await c.fetchone():
                return
        # Проверка, что реферер существует и это не сам юзер
...
async def check_all(user_id):
    """Возвращает (сколько_новых_подписок, общая_сумма_награды)."""
...
# ========== ХЭНДЛЕРЫ ==========
_last = {}
_states = {}  # для ввода значений админом
...
async def adm_stats(cq: types.CallbackQuery):
    ...
    await cq.message.edit_text(
        f"📊 <b>Статистика</b>\n\n"
        f"👥 Юзеров: <b>{total}</b>\n"
        f"💰 Суммарный баланс юзеров: <b>${balances:.4f}</b>\n"
        f"✅ Подписок: <b>{subs_cnt}</b>\n"
        f"💵 Начислено: <b>${paid:.4f}</b>\n"
        f"💸 Заявок на вывод (pending): <b>{pend}</b>",
